...
class MyApp(MDApp):
    ...
    def load_habits(self):
        """Loads habits from JSON when the app starts"""
    ...
    def save_habits(self):
        """Saves habits and UI data to JSON"""
        data = {}
    ...
    def toggle_day_selection(self, button):
        """Toggles selection of a day and filters displayed habits."""
        day = button.text
    ...
    def add_habit(self, habit_name):
        """Adds a new habit and ensures it's saved correctly."""
    ...
    def add_habit_card(self, habit_name, day):
        """Creates and displays a habit card without duplicates."""
        for card in self.root.ids.habits_list.children:
        ...
        self.root.ids.habits_list.height += card.height  # Adjust height

    def delete_habit(self, habit_name, day, card):
        """Deletes a habit and updates the UI"""
        if day in self.habits and habit_name in self.habits[day]:
            ...
            self.root.ids.habits_list.remove_widget(card)  # Remove habit card from UI
            self.save_habits()  # Save the updated habits list

    def filter_habits(self):
        """Shows only unique habits associated with selected days and removes empty space."""
        habits_list = self.root.ids.habits_list
        ...
        # 🔥 **Fix: Dynamically update height to remove invisible space**
